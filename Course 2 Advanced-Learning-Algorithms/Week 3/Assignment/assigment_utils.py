...
def plt_tune_regularization(X_train, y_train, X_cv, y_cv, x, y_pred, err_train, err_cv, optimal_reg_idx, lambda_range):
    ...
    # *ax[1].get_ylim() ensures that the lower and upper limits of the y-axis are passed as separate arguments to
    # vlines, defining the extent of the vertical line to be drawn. ax[1].get_ylim(): This retrieves the current
    # limits of the y-axis on the second subplot, returning them as a tuple (e.g., (lower_limit, upper_limit)).
...
def tune_m():
    """ tune the number of examples to reduce over-fitting """
    m = 50
    m_range = np.array(m * np.arange(1, 16))  # This converts the resulting list to a NumPy array
    num_steps = m_range.shape[0]
    degree = 16
    err_train = np.zeros(num_steps)
    err_cv = np.zeros(num_steps)
    y_pred = np.zeros((100, num_steps))  # x=100 the data to be predicted
...
def plt_mc_data(ax, X, y, classes, class_labels=None, map=plt.cm.Paired, legend=False, size=50, m='o'):
    ...
    for i in range(classes):
        idx = np.where(y == i)  # return indices where class = i
        label = class_labels[i] if class_labels else "c{}".format(i)
        # This allows for the flexibility of providing custom labels for each class (class_labels), and if custom
        # labels are not provided, it defaults to labeling the classes as "c0", "c1", "c2", and so on, based on their
        # numerical index.
...
def plot_cat_decision_boundary(ax, X, predict, class_labels=None, legend=False, vector=True, color='g', lw=1):
    ...
    # xx.ravel() flattens the 2D array xx into a 1D array, preserving the row-major order.
    # yy.ravel() similarly flattens the 2D array yy into a 1D array.
    #
    # np.c_[] then concatenates these flattened arrays along the second axis, effectively combining the x-coordinates
    # and y-coordinates of each point into a single 2D array.
    #
    # As a result, points contains pairs of x and y coordinates corresponding to each point in the grid defined by xx
    # and yy. Each row of points represents a point in the grid, with the first column containing the x-coordinate
    # and the second column containing the y-coordinate.
    ...
    # Z defines the function values at each X-Y coordinate, and the contour lines are drawn based on these function
    # values to visually represent the shape and characteristics of the function.
    ax.axis('tight')
...
def plt_train_eq_dist(X_train, y_train, classes, X_cv, y_cv, centers, std):
    ...
    fig.canvas.header_visible = False
    fig.canvas.footer_visible = False
    plt_mc_data(ax[0], X_train, y_train, classes, map=dkcolors_map, legend=True, size=50, m='o')
    ...
    for c in css:
        ...
        ax[0].add_patch(circ)  # adds the circle patch to the first subplot (ax[0])

    # make a model for plotting routines to call (This function is to plot contour lines )
    ...
    # vector = False because cat_predict calculates point by point. If vector is True, it expects the predict function
    # to accept vectorized input and returns predictions for all points in the meshgrid at once.If vector is False,
    # it iterates over each point in the meshgrid and makes individual predictions.
...
def plt_nn(model_predict, X_train, y_train, classes, X_cv, y_cv, suptitle=""):
    ...
    # this draws the contur plot over the predicted training data using nn
